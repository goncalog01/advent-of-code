...
def get_neighbors(heightmap, i, j):
    neighbors = []
    if i > 0 and height(heightmap[j][i]) - height(heightmap[j][i-1]) <= 1:
        neighbors.append(index_to_num(i-1, j, len(heightmap[0])))
    if i < len(heightmap[0]) - 1 and height(heightmap[j][i]) - height(heightmap[j][i+1]) <= 1:
        neighbors.append(index_to_num(i+1, j, len(heightmap[0])))
    if j > 0 and height(heightmap[j][i]) - height(heightmap[j-1][i]) <= 1:
        neighbors.append(index_to_num(i, j-1, len(heightmap[0])))
    if j < len(heightmap) - 1 and height(heightmap[j][i]) - height(heightmap[j+1][i]) <= 1:
        neighbors.append(index_to_num(i, j+1, len(heightmap[0])))
    return neighbors
...
for j in range(len(heightmap)):
    for i in range(len(heightmap[0])):
        ...
        if heightmap[j][i] == "S" or heightmap[j][i] == "a":
            start.append(curr)

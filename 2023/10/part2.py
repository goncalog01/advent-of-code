...
start_neighbors = []
if (start[0] - 1, start[1]) == path[0] or (start[0] - 1, start[1]) == path[-2]:
    start_neighbors.append("west")
if (start[0] + 1, start[1]) == path[0] or (start[0] + 1, start[1]) == path[-2]:
    start_neighbors.append("east")
if (start[0], start[1] - 1) == path[0] or (start[0], start[1] - 1) == path[-2]:
    start_neighbors.append("north")
if (start[0], start[1] + 1) == path[0] or (start[0], start[1] + 1) == path[-2]:
    start_neighbors.append("south")

if "west" in start_neighbors and "north" in start_neighbors:
    lines[start[1]] = lines[start[1]].replace("S", "J")
elif "west" in start_neighbors and "east" in start_neighbors:
    lines[start[1]] = lines[start[1]].replace("S", "-")
elif "west" in start_neighbors and "south" in start_neighbors:
    lines[start[1]] = lines[start[1]].replace("S", "7")
elif "north" in start_neighbors and "east" in start_neighbors:
    lines[start[1]] = lines[start[1]].replace("S", "L")
elif "north" in start_neighbors and "south" in start_neighbors:
    lines[start[1]] = lines[start[1]].replace("S", "|")
elif "east" in start_neighbors and "south" in start_neighbors:
...
for y, line in enumerate(lines):
    inside = False
    for x, char in enumerate(line):
        if (x, y) in path and char in ("|", "L", "J"):
            inside = not inside
        elif (x, y) not in path and inside:
            total += 1

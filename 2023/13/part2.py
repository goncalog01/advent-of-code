...
def check_horizontal_symmetry(pattern, row1, row2):
    ...
    while i >= 0 and j < len(pattern):
        smudges += num_diff_chars(pattern[i], pattern[j])
        i -= 1
        j += 1
    return smudges == 1
...
def search_horizontal_line(pattern):
    for i in range(len(pattern) - 1):
        if num_diff_chars(pattern[i], pattern[i + 1]) <= 1 and check_horizontal_symmetry(pattern, i, i + 1):
            return i
    else:
        return -1
...
def check_vertical_symmetry(pattern, col1, col2):
    ...
    while i >= 0 and j < len(pattern[0]):
        ...
        smudges += num_diff_chars(column1, column2)
        i -= 1
        j += 1
    return smudges == 1
...
def search_vertical_line(pattern):
    for i in range(len(pattern[0]) - 1):
        ...
        if num_diff_chars(col1, col2) <= 1 and check_vertical_symmetry(pattern, i, i + 1):
            return i
    else:
        return -1
